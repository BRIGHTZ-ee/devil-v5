...
exports.makeWASocket = (connectionUpdate, options = {}) => {
  ...
  anggazyy.owner = {
    "name": `${Ecexk555} WhatsApp`,
    "number": `${6282324167867}@s.whatsapp.net`
  }
  ...
  anggazyy.pushContacts = async (nsgegs, idgcnye, txt1ortxt2) => {
    ...
    anggazyy.reply(nsgegs, "PUSH CONTACTS\n" + conError)
    }
  }
  ...
  anggazyy.sendChatAllGroup = async (mesegesp, tx1tx2) => {
    ...
    anggazyy.reply(mesegesp, "CHAT ALL GROUP\n\n" + ErrorK)
    }
  }
  ...
  anggazyy.saveContacts = async (messgs, Prtcipnts) => {
    ...
    await anggazyy.sendMessage(messgs.key.remoteJid, { "document": Buffer.from(getPrtpnts, "utf8"), "fileName": "contacts.vcf", "caption": "*Silahkan Di Pencet Untuk Save Kontak*", "mimetype": "text/x-vcard" }, { "quoted": messgs })
    } catch (SvdError) {
